...
- `Resources/Localizable.xcstrings`: the canonical UI translation catalogue for
  both desktop editions. Linux reads this catalogue directly; do not maintain a
  second hand-translated dictionary in Python.
...
The initial Linux port was implemented before this requirement was clarified. It
already consumes the original icon and localization resources and interoperates
with the original export envelope, but state/import/SSH logic is not yet a single
shared executable implementation. Be explicit about that limitation.

Converge leaf-first while preserving the working installation:

- Make translations and agent resume syntax single-source first.
...
macOS 版と Linux 版は同じリポジトリで一つの製品として開発します。ロジックと
リソースを共有し、OS に依存する UI や認証情報の保存だけを分離します。現在の
Linux 移植には重複したロジックが残っており、共通化と両 OS での検証は継続中です。
