...
iOS camera service for UniConnect mobile QR-code pairing.
...
- ``QRCodeScanStream`` — an `AsyncStream<String>` of decoded QR payloads, fed by
  ...
  injected predicate (UniConnect pairing only accepts `uniconnect://` links).
...
#expect(seen == ["uniconnect://example"])
```
