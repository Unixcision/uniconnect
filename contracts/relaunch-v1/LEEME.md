...
| `plan-request.json`, `apply-request.json`, `apply-response.json`, `apply-recovered-response.json`, `apply-in-progress-response.json`, `status-request.json`, `errors.json` | Nadie todavía. |

Linux no lee ninguno: el motor de relanzado de la rama `desarrollo/relaunch-linux-20260914` todavía
no está portado a esta.

Causas añadidas: `sin_ia` (24-09-2026), para una ventana sin ninguna IA en marcha. Antes llegaba ya
en `plan-all-excluded-response.json` y Android la conservaba como identificador sin traducir.
