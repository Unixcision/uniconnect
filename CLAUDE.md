...
- **Never pass `.contains` as an unapplied method reference** (`allSatisfy(set.contains)`, `contains(where: CharacterSet.newlines.contains)`). Swift 6.4 (Xcode 27) miscompiles it at `-O` when a file references the `contains` of two different `CharacterSet`s: one silently uses the other set. Debug and the unit tests (`-Onone`) stay green, so only the Release app breaks; in 9f99c015f it made the SSH validator reject every saved connection and every SSH window came back disconnected. Write `{ set.contains($0) }`. `scripts/lint-unapplied-contains.sh` enforces it in CI.
