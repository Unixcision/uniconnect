...
fun BoxActionsSheet(
    ...
    ModalBottomSheet(onDismissRequest = onDismiss, sheetState = rememberModalBottomSheetState(skipPartiallyExpanded = true), containerColor = Brand.Surface) {
        Column(Modifier.padding(horizontal = 12.dp).padding(bottom = 28.dp)) {
            SectionLabel(stringResource(kind)) {}
            Text(title, Modifier.padding(horizontal = 8.dp, vertical = 6.dp), style = MaterialTheme.typography.titleLarge, fontWeight = FontWeight.SemiBold, maxLines = 1, overflow = TextOverflow.Ellipsis)
...
private fun Action(label: Int, icon: ImageVector, onClick: () -> Unit) {
    ListItem(
        ...
        leadingContent = { Icon(icon, null, tint = Brand.Cyan) },
