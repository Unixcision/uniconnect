    ...
    private var currentBannerText: String? {
        ...
        case .cmux:
            return String(
                ...
                defaultValue: "This is the UniConnect Ghostty config selected for this build. Edit it here, then Save to reload UniConnect."
            )
        case .synced:
            if currentSnapshot.hasStandaloneGhosttyConfig {
                return String(
                    localized: "settings.config.banner.synced",
                    defaultValue: "This is a generated preview of the effective config. Edit the UniConnect tab to change what UniConnect reads."
                )
            }
            return String(
                localized: "settings.config.banner.syncedNoGhostty",
                defaultValue: "This is a generated preview of the effective config. No base Ghostty config file was found, so only UniConnect overrides are shown."
            )
        }
    }
    ...
    private func saveCmuxConfig() {
        ...
        do {
            ...
            statusMessage = String(
                ...
                defaultValue: "Saved to the UniConnect config and reloaded."
            )
            statusIsError = false
        } catch {
            NSSound.beep()
            statusMessage = String(
                ...
                defaultValue: "Couldn't save the UniConnect config."
            )
            statusIsError = true
        }
    }
    ...
    private func materializedCmuxConfigURL() -> URL? {
        ...
        do {
            ...
            statusMessage = String(
                ...
                defaultValue: "Couldn't open the UniConnect config."
            )
            statusIsError = true
            return nil
        }
...
private extension ConfigSource {
    var localizedTitle: String {
        switch self {
        case .cmux:
            return String(localized: "settings.config.source.cmux", defaultValue: "UniConnect")
        case .synced:
