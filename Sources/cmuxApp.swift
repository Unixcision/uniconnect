...
struct cmuxApp: App {
    ...
    private static func terminateForMissingLaunchTag() -> Never {
        let message = "error: refusing to launch untagged UniConnect DEV; start with ./scripts/reload.sh --tag <name> (or set CMUX_TAG for test harnesses)"
        fputs("\(message)\n", stderr)
        fflush(stderr)
        NSLog("%@", message)
        Darwin.exit(64)
...
typealias AppLanguage = CmuxSettings.AppLanguage

// Apply only the app-scoped preference; never change the system or terminal locale.
enum LanguageSettings {
    static let languageKey = "appLanguage"
    static let defaultLanguage: AppLanguage = .es

    static func apply(_ language: AppLanguage) {
        let effective = language.effectiveLanguage.rawValue
        UserDefaults.standard.set(effective, forKey: languageKey)
    ...
    static var languageAtLaunch: AppLanguage {
        ...
        return (stored.flatMap(AppLanguage.init(rawValue:)) ?? .es).effectiveLanguage
    }
}
