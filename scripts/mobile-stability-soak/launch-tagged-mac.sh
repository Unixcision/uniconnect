...
app="${CMUX_SWAPP:-$HOME/Library/Developer/Xcode/DerivedData/uniconnect-${tag}/Build/Products/Debug/UniConnect DEV ${tag}.app}"
port="${CMUX_PORT:-9300}"
port_range="${CMUX_PORT_RANGE:-10}"
port_end="${CMUX_PORT_END:-$((port + port_range - 1))}"
dev_origin="${CMUX_DEV_ORIGIN:-http://localhost:${port}}"
bin="$app/Contents/MacOS/UniConnect DEV"
...
exec env \
  ...
  CMUXD_UNIX_PATH="$HOME/Library/Application Support/UniConnect/cmuxd-dev-${tag}.sock" \
