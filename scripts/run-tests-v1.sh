...
APP="$DERIVED_DATA_PATH/Build/Products/Debug/UniConnect DEV.app"
RUN_TAG="tests-v1"
...
if [ ! -d "$APP" ]; then
  echo "ERROR: UniConnect DEV.app not found at expected path: $APP" >&2
  exit 1
fi
...
launch_and_wait() {
  ...
  for _ in {1..50}; do
    pgrep -x "UniConnect DEV" >/dev/null 2>&1 || break
    sleep 0.1
  done

  # Force socket mode for deterministic automation runs, independent of prior user settings.
  defaults write com.unixcision.uniconnect.debug socketControlMode -string full >/dev/null 2>&1 || true

  # Launch directly with UI test mode enabled so startup follows deterministic test codepaths.
  CMUX_TAG="$RUN_TAG" CMUX_UI_TEST_MODE=1 "$APP/Contents/MacOS/UniConnect DEV" >/dev/null 2>&1 &
