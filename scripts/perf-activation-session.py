...
class CmuxPerfRunner:
    def __init__(self, args: argparse.Namespace):
        ...
        self.socket_path = pathlib.Path(f"/tmp/uniconnect-debug-{self.tag_slug}.sock")
        self.cmuxd_socket_path = pathlib.Path(
            os.path.expanduser(f"~/Library/Application Support/UniConnect/cmuxd-dev-{self.tag_slug}.sock")
        )
        self.debug_log_path = pathlib.Path(f"/tmp/uniconnect-debug-{self.tag_slug}.log")
        self.stdout_path = pathlib.Path(f"/tmp/uniconnect-perf-{self.tag_slug}-stdout.log")
        self.app_path = pathlib.Path(args.app_path).expanduser() if args.app_path else self.default_app_path()
        self.binary_path = self.app_path / "Contents/MacOS/UniConnect DEV"
    ...
    def make_fixture_root(self, fixture_root_arg: str) -> pathlib.Path:
        if fixture_root_arg:
            ...
            return pathlib.Path(tempfile.mkdtemp(prefix=f"uniconnect-perf-{self.tag_slug}-", dir=str(fixture_parent)))
        return pathlib.Path(tempfile.mkdtemp(prefix=f"uniconnect-perf-{self.tag_slug}-"))

    def default_app_path(self) -> pathlib.Path:
        return pathlib.Path.home() / (
            f"Library/Developer/Xcode/DerivedData/uniconnect-{self.tag_slug}/"
            f"Build/Products/Debug/UniConnect DEV {self.tag_slug}.app"
        )
    ...
    def clean_persisted_state(self) -> None:
        app_support = pathlib.Path.home() / "Library/Application Support/UniConnect"
        bundle_id = f"com.unixcision.uniconnect.debug.{self.tag_id}"
        ...
        env.update(
            {
                ...
                "CMUX_TAG": self.tag,
                "CMUX_BUNDLE_ID": f"com.unixcision.uniconnect.debug.{self.tag_id}",
            }
        )
        return env
    ...
    def cli_env(self) -> dict[str, str]:
        ...
        env["CMUX_BUNDLE_ID"] = f"com.unixcision.uniconnect.debug.{self.tag_id}"
        ...
        if proc and proc.poll() is None:
            ...
            try:
                ...
                try:
                    ...
                    pass
        subprocess.run(
            ["pkill", "-f", re.escape(f"UniConnect DEV {self.tag_slug}.app/Contents/MacOS/UniConnect DEV")],
