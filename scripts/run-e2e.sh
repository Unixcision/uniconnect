...
REPO="${UNICONNECT_GITHUB_REPOSITORY:-Unixcision/uniconnect}"
RESULTS_REPO="${UNICONNECT_E2E_RESULTS_REPOSITORY:-$REPO}"
...
if [ "$WAIT" = true ]; then
  ...
  ISSUE_URL=$(gh search issues "$RUN_ID" --repo "$RESULTS_REPO" --limit 1 --json url --jq '.[0].url' 2>/dev/null || true)
  if [ -n "$ISSUE_URL" ]; then
