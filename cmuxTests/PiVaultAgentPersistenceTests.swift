...
final class PiVaultAgentPersistenceTests: XCTestCase {
    ...
    func testBuiltInAntigravityRegistrationLoadsHistoryDisplayAndWorkspace() async throws {
        ...
        XCTAssertEqual(
            entry.resumeCommand,
            "{ cd -- '/tmp/antigravity repo' 2>/dev/null || [ ! -d '/tmp/antigravity repo' ]; } && 'agy' '--dangerously-skip-permissions' '--conversation' 'antigravity-conversation-123'"
        )
    }
    ...
    func testBuiltInAntigravityRegistrationIndexesEachHistoryConversation() async throws {
        ...
        XCTAssertEqual(
            ...
            "{ cd -- '/tmp/antigravity repo' 2>/dev/null || [ ! -d '/tmp/antigravity repo' ]; } && 'agy' '--dangerously-skip-permissions' '--conversation' 'conversation-b'"
        )
    }
