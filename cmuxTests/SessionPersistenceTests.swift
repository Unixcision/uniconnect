...
final class SessionPersistenceTests: XCTestCase {
    ...
    func testRestoredAntigravityAgentAutoResumeUsesConversationCommand() throws {
        ...
        XCTAssertEqual(
            agent.resumeCommand,
            "{ cd -- '/tmp/repo' 2>/dev/null || [ ! -d '/tmp/repo' ]; } && '/usr/local/bin/agy' '--dangerously-skip-permissions' '--conversation' 'antigravity-conversation-123' '--sandbox' 'danger-full-access'"
        )
    }
...
final class SocketListenerAcceptPolicyTests: XCTestCase {
    ...
    func testCodexResumeCommandDropsStartupImagesAndPlacesSessionBeforeFlags() {
        ...
        XCTAssertEqual(
            snapshot.resumeCommand,
            "{ cd -- '/Users/lawrence/fun/cmuxterm-hq' 2>/dev/null || [ ! -d '/Users/lawrence/fun/cmuxterm-hq' ]; } && '/Users/lawrence/.bun/bin/codex' '--yolo' 'resume' '019e2bb9-5544-7201-a517-d77bb00d724f' '--model' 'gpt-5.4'"
        )
    }
    ...
    func testRestorableAgentIndexLoadsLaunchCommandFromHookStore() throws {
        ...
        XCTAssertEqual(
            snapshot.resumeCommand,
            "{ cd -- '/tmp/repo' 2>/dev/null || [ ! -d '/tmp/repo' ]; } && 'env' 'CODEX_HOME=/tmp/codex' '/usr/local/bin/codex' '--yolo' 'resume' 'codex-session-123' '--model' 'gpt-5.4' '--search'"
        )
    }
