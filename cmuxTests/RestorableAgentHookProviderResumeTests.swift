...
extension SocketListenerAcceptPolicyTests {
    ...
    func testAntigravityResumeCommandUsesConversationAndDropsStartupSelectors() {
        ...
        XCTAssertEqual(
            snapshot.resumeCommand,
            "{ cd -- '/tmp/antigravity repo' 2>/dev/null || [ ! -d '/tmp/antigravity repo' ]; } && 'env' 'GEMINI_CLI_HOME=/tmp/gemini home' '/Users/example/.local/bin/agy' '--dangerously-skip-permissions' '--conversation' 'antigravity-conversation-123' '--sandbox' 'danger-full-access' '--add-dir' '/tmp/extra repo'"
        )
    }
    ...
    func testAdditionalHookAgentResumeCommandsUseVerifiedCLIResumeFlags() {
        ...
        let copilot = SessionRestorableAgentSnapshot(
            ...
            launchCommand: AgentLaunchCommandSnapshot(
                launcher: "copilot",
                executablePath: "/tmp/uniconnect-agent-upstreams/copilot-install/bin/copilot",
                arguments: [
        ...
        XCTAssertEqual(
            ...
            "{ cd -- '/tmp/copilot repo' 2>/dev/null || [ ! -d '/tmp/copilot repo' ]; } && 'env' 'COPILOT_HOME=/tmp/copilot home' '/tmp/uniconnect-agent-upstreams/copilot-install/bin/copilot' '--resume' 'copilot-session-123' '--model' 'gpt-5.4' '--allow-all-tools'"
        )
        XCTAssertEqual(
